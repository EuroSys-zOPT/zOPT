...
private:
    ...
    // typedef set<RIHeapElement, RIHeapElementComparator> RIHeap;
...
public:
    ...
    : histogram(_histogram), heap() {
        ...
        // cout << "RI Heap init. completed" << endl;
    }
...
public:
    LeaseCache(ULL _cache_size, double alpha, RIHistogram &_histogram, RIHeapCollection &_ri_heap) 
        ...
        // index_t bri_max = ceil(log2((double)R));
        // if(!(R & (R-1))) bri_max++;

        // // for(ULL i=1; i<R; i*=2)
        // for(ULL i=0; i<=bri_max; i++)
        //     buckets.push_back(new BucketGroup((ULL)pow(2,i), alpha, it_map, 
        //     i != 0 ? buckets.at(buckets.size()-1) : NULL));
        // for(ULL i=0; i<bri_max; i++)
...
list<offset_t>::iterator Bucket::insert_block(offset_t block) {
    ...
    //     group->update_it_map(my_idx);
    // }
    // assert(block);
    // buffer.push_back(block);
    // return buffer.begin()+(buffer.size()-1);
...
void BucketGroup::flush_bucket(int bucket_num) {
    // cout << "flush_bucket " << L << "," << bucket_num << endl;
...
// void BucketGroup::update_it_map(int bucket_num) {
//     auto &buf = buckets[bucket_num].get_buffer();
//     for(auto i=buf.begin(); i != buf.end(); i++) {
//         // assert(*i);
//         it_map[*i] = {&buckets[bucket_num], i};
//     }
// }

// void BucketGroup::update_it_map(int bucket_num, list<offset_t>::iterator start_it) {
//     auto &buf = buckets[bucket_num].get_buffer();
//     for(auto it=start_it; it != buf.end(); it++) {
//         it_map[*it] = {&buckets[bucket_num], it};
//     }
// }
...
void BucketGroup::insert_buffer_entries(list<offset_t> &buf) {
    ...
    // ULL add_size = buf.size();
    
    // auto i = ins_buf.begin() + before_size;
    // while(before_size + add_size >= ins_buf.capacity()) {
    //     ins_buf.reserve(ins_buf.capacity() * 2);
    //     i = ins_buf.begin();
    // }
    
    // ins_buf.insert(ins_buf.end(), buf.begin(), buf.end());
    // auto start = ins_buf.begin();
    // for(int i=0; i<before_size; i++) start++;

    // update_it_map(insert_idx, start);
...
ULL RIHistogram::cost(offset_t block, index_t lease) {
    ...
    // for(index_t i=0; i<=lease; i++) {
    //     current += (i * histogram[block][i]);
    // }

    // for(index_t i=lease+1; i<histogram[block].size(); i++) {
    //     after += (lease * histogram[block][i]);
    // }
...
void RIHeapCollection::update_ppuc(offset_t block, index_t lease) {
    // vector<RIHeap::iterator> its;
    // for(auto it = heap.begin(); it != heap.end(); it++) {
    //     if(it->block == block) its.push_back(it);
    // }
    ...
    //     if(it->first > lease) {
    //         double ppuc = histogram.get_ppuc(block, lease, it->first);
    //         heap.insert({block, it->first, ppuc});
    //     }
    // }
...
void LeaseCache::process_ppuc() {
    ...
    // if(assigned_leases[0] == 0) {
    //     for(int i=0; i<assigned_leases.size(); i++)
    //         assigned_leases[i]++;
    // }
...
PPUCBlock LeaseCache::get_max_ppuc() {
    ...
    // PPUCBlock temp;
    // for(offset_t block=1; block<histogram.get_data_block_count(); block++) {
    //     temp = histogram.get_best_ppuc_of_block(block, assigned_leases[block]);
    //     if(!temp.finished && best_block.ppuc < temp.ppuc) {
    //         best_block = temp;
    //     }
    // }
...
index_t LeaseCache::get_group_num(index_t lease) {
    ...
    // If lease is power of two,
    // return ret+1, else return ret
...
int main(int argc, char** argv)
{
    ...
    // if(CACHE_MIN_SIZE == 0) {
    //     CACHE_MIN_SIZE = ceil((double)CACHE_MAX_SIZE/(double)(BLOCK_SIZE*step_count)) * BLOCK_SIZE;
    // }
    ...
    // RIHeapCollection base_heap(hist);
    ...
    for(int i=1; i<=step_count; i++) {
        ...
        // RIHeapCollection heap(base_heap);
    ...
    // cout << endl << "Logical hit ratio:" << endl;
    // cache.debug_cache_logical_stat();
