...
// ctpl::thread_pool tp(MAX_THREAD);
class NextAccessMap {
private:
...
public:
  ...
  map(max_val+1, NULL) {
    // distinct_blocks = count_distinct_datablock(0, access_count);
...
private:
  ...
  // multiset<index_t, greater<index_t>> tree;
...
private:
  ...
  //ULL *hit, *miss;
  ULL hit=0, miss=0;
  // vector<atomic<ULL> *> hit, miss;
  ULL cold_miss=0;

  bool *ref_map;

  // void create_and_run_part_cache_with_affinity(int cache_num) {
  //   while(!affinity_set[cache_num]);
  //   // cout << "PCache "+to_string(cache_num)+" initilizing...\n"; 
  //   auto p = new PartCache(this, cache_num, step_size/block_size, access_map.get_access_count(), false);
  //   __atomic_exchange_n(&caches[cache_num], p, __ATOMIC_RELAXED);
  //   if(cache_num != part_cache_num-1){
  //     while((PartCache *)__atomic_load_n(&caches[cache_num+1], __ATOMIC_RELAXED) == NULL);
  //     caches[cache_num]->set_next_level(caches[cache_num+1]);
  //   }      
  //   // cout << "PCache "+to_string(cache_num)+" initilize complete.\n"; 
  
  //   caches[cache_num]->run_thread();
  // }
...
public:
  ...
  caches(_part_cache_num, NULL), block_size(_block_size), thread_num(_thread_num) {
    ...
    if(min_size == max_size) { // Single size simulation
      ...
      // step_size = (double)(max_size/block_size)/(double)thread_num * block_size;
      ...
      if(thread_num-1 > 0) {
        ...
        //ULL last_step_size = last_step_pcache_size/block_size;
      ...
      for(int i=1; i<part_cache_num; i++) {
        // cout << "Checking PCache "+to_string(i)+"\n"; 
        while((PartCache *)__atomic_load_n(&caches[i], __ATOMIC_RELAXED) == NULL);
        // cout << "PCache "+to_string(i)+" init. checked\n"; 
      }
      if(part_cache_num > 1)
        caches[0]->set_next_level(caches[1]);
        // caches.push_back(new PartCache(this, i, step_size/block_size, access_map.get_access_count()));
      return;
    } 
    // else {
    //   step_size = part_cache_num > 1 ? ((index_t)ceil((double)((max_size - min_size)/block_size)/(double)(part_cache_num - 1)))*block_size : 0;
    ...
    //hit = (ULL *)calloc(part_cache_num, sizeof(ULL));
    //miss = (ULL *)calloc(part_cache_num, sizeof(ULL));
    /*for(int i=0; i<part_cache_num; i++) {
      hit.push_back(new atomic<ULL>(0));
      miss.push_back(new atomic<ULL>(0));
    }*/
  ...
  ~MultiCache() { 
    ...
    // for(auto i : hit) delete i;
    // for(auto i : miss) delete i;
    free(ref_map);
  }

  void hit_from(index_t cache_num) {
    //for(index_t i=cache_num; i<part_cache_num; i++)
      // (*hit[i])++;
    __atomic_add_fetch(&hit, 1, __ATOMIC_RELAXED);
  }
  void miss_in(index_t cache_num) {
    // (*miss[cache_num])++;
    // __atomic_add_fetch(&miss, 1, __ATOMIC_RELAXED);
    if(cache_num == part_cache_num - 1)
      miss++;
  }
  ...
  void assign_task(index_t cache_num, index_t cur_access, index_t put_entry, bool is_cold) { 
    if(cache_num < part_cache_num - 1) {
      ...
      // caches[cache_num]->search_and_put(cur_access, put_entry, is_cold);
    }
  }
...
void NextAccessMap::init(index_t begin, index_t end) {
  // bool is_exist[max_val] = {false, };
  // offset_t cur_offset;
  // for(long long i = 0; i < access_log->size(); i++) {
  //   cur_offset = access_log->at(i);
  //   if(!is_exist[cur_offset]) map.insert(make_pair(cur_offset, new TNRQueue()));
...
void NextAccessMap::init_with_threads(int num_thread) {
  if(num_thread == 1) {
    init(0, access_count);
    return;
  } else if(num_thread > CORE_THREAD) {
...
void MultiCache::access(offset_t offset) {
  ...
  if(part_cache_num > 1) {
    ...
    auto start = chrono::high_resolution_clock::now();
#endif
    // caches[1]->put_work(cur_access, evicted_block, is_cold);
...
void MultiCache::print_statistics() {
  ...
  // ULL cache_hit = caches[part_cache_num-1]->get_hit();
  // ULL cache_miss = caches[part_cache_num-1]->get_miss();
  // cold_miss = caches[part_cache_num-1]->get_cold_miss();
...
index_t PartCache::put(index_t cur_access) {
  ...
  tree.push(cur_access);
  // index_t evicted_block = *tree.begin();
  // tree.erase(tree.begin());
  // tree.insert(cur_access);
...
void PartCache::search_and_put(index_t cur_access, index_t put_entry, bool is_cold) {
  ...
  if(search(cur_access)) { // Hit from current cache size
    ...
    // parent_cache->hit_from(part_num);
    cache_hit();
    ...
    // if(!is_cold) parent_cache->miss_in(part_num);
    
    index_t new_put_entry = put(put_entry);
    if(new_put_entry != EMPTY_ENTRY)
      // parent_cache->assign_task(part_num+1, cur_access, new_put_entry, is_cold);
...
void PartCache::worker_function() {
  ...
  while(get_total_processed() < total_access_count) {
    ...
    // if(wq.try_dequeue(w)) {
    //   if(w.cur_access == EMPTY_WORK) {
    //     if(!w.is_cold) cache_hit();
    //     else cache_cold_miss();
    //     put_work_to_next_level(w.cur_access, w.put_entry, w.is_cold);
    //   }
    //   else search_and_put(w.cur_access, w.put_entry, w.is_cold);
    // }
    
  }
  // parent_cache->join_part_cache_thread(part_num);
#ifdef THREAD_UPTIME_LOG
  auto elapsed = chrono::high_resolution_clock::now() - start;
...
void calculate_optimal_hit_ratio(vector<offset_t> *access_log) {
  ...
  auto elapsed = chrono::high_resolution_clock::now() - start;
  // Cache my_cache(CACHE_SIZE, BLOCK_SIZE, max_value, my_access_map);
  ...
  // my_cache.start_part_cache_threads();
  ...
  // for(index_t i=0; i < STEP_COUNT; i++)
  //   my_cache.join_part_cache_thread(i);
  elapsed = chrono::high_resolution_clock::now() - start;
...
ULL extract_access_log(ifstream &log_file, vector<offset_t> &access_log) {
  ...
  // if(repeat_trace_time > 1) {
  //   auto old_size = access_log.size();
  //   access_log.reserve(old_size * repeat_trace_time);
  //   for(int i=1; i<repeat_trace_time; i++) {
  //     copy_n(access_log.begin(), old_size, back_inserter(access_log));
  //   }
  // }   
...
int main(int argc, char** argv) {
  ...
  // cout << "Max thread #: " << MAX_THREAD << endl;
